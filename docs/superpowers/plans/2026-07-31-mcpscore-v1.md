...
export interface RepoSnapshot {
  ...
  commitsLast90Days?: number       // undefined when the commits listing could not be fetched (absence ≠ zero)
  busFactor?: number               // authors with >=3 commits in trailing 365d sample; undefined on fetch failure
  medianIssueResponseDays?: number // undefined without token (rate-limit protection)
  treePaths?: string[]             // undefined when the tree could not be fetched (distinct from an empty repo)
