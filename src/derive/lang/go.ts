...
//   3. mark3labs positional: `MustTool("x", "desc", ...)` / `NewTool("x", "desc")`
...
// NewTool("x","desc") with no WithDescription option:
//   MustTool("search_dashboards", "Search for Grafana dashboards by title, tag, or folder", handler)
const GO_POSITIONAL_RE = /\b(?:MustTool|NewTool)\(\s*["'`]([\w.-]+)["'`]\s*,\s*["'`]([^"'`]*)["'`]/g
...
// are naturally disjoint on well-formed input.
const GO_NEWTOOL_CALL_RE = /\bNewTool\(\s*["'`]([\w.-]+)/g
